...
const dataPath = path.join(root, "data", "v8-execution-dashboard-data.json");
const outputPath = path.join(root, "docs", "v8.html");
const RUN_DATE = new Date().toLocaleDateString("en-CA", { timeZone: "Asia/Seoul" });
const HISTORY_PAGES = 13;
...
const HOLDER_COST_DIRECT_INPUTS = new Map([
  // Tier A slot: add DART transaction-price evidence here when a disclosure gives exact acquisition/disposal prices.
  // ["000000", { holder: "Holder name", estimatedCost: 10000, asOf: "2026-01-01", sourceUrl: "https://...", memo: "DART direct weighted-average price" }]
]);
...
async function main() {
  ...
  for (const [index, item] of universe.entries()) {
    try {
      const daily = await fetchDailyHistory(item.ticker, HISTORY_PAGES);
      const flow = await fetchForeignHistory(item.ticker, HISTORY_PAGES);
  ...
  applyHolderCostScores(ranked);
  const growthFundReview = buildGrowthFundReview(ranked);
  const holderRankList = ranked
    .toSorted((a, b) => b.v8cScore - a.v8cScore || b.holderCostScore - a.holderCostScore || b.totalScore - a.totalScore)
    .slice(0, 35);
  ...
  const output = {
    meta: {
      title: "국민성장펀드 v8 대주주 평단 보조축 대시보드",
      runDate: RUN_DATE,
      universeCount: universe.length,
      validCount: valid.length,
      methodology: "v8은 기존 v7 totalScore와 decision을 그대로 유지하면서 대주주·기관성 자금의 추정 평단가를 별도 보조축으로 추가한다. Tier A는 공시 직접 단가, Tier B는 최근 외국인·기관 순매수 구간 VWAP, Tier C는 프록시·NO_DATA로 분리하고, 신뢰도 계수로 holderCostScore를 보수적으로 제한한다.",
      warning: "투자 권유가 아니라 후보 탐색과 실행 조건 점검표입니다. holderCostScore는 단독 매수 신호가 아니라 현재가가 스마트머니 추정 원가 근처인지, 수익권 오버행이 있는지 확인하는 보조축입니다."
    },
...
function evaluate(item, history, quote) {
  ...
  const holderFlow = estimateHolderFlowCost(history, quote);
  ...
  const plan = entryPlan(decision, latest.close, ma5, ma20, ma60, low60, high60, rsi14);
  return {
    ...item,
    ...
    decision,
    entryPlan: plan,
    executionPlan: executionPlan(decision, latest.close, ma5, ma20, ma60, low60, high60, rsi14, ret5, ret20, flowScore, risk),
...
function executionPlan(decision, close, ma5, ma20, ma60, low60, high60, rsi14, ret5, ret20, flowScore, risk) {
...
function summarize(rows) {
  ...
  const holderCount = (signal) => rows.filter((row) => row.holderSignal === signal).length;
  const bySector = {};
  ...
  return {
    ...
    holderSignals: {
      accumulation: holderCount("ACCUMULATION"),
      neutral: holderCount("NEUTRAL"),
      overhang: holderCount("OVERHANG"),
      noData: holderCount("NO_DATA")
    },
    bySector
  };
}

function estimateHolderFlowCost(history, quote) {
  const rows = history.slice(-126).filter((row) => row.close != null);
  let netShares = 0;
  let netValueEok = 0;
  let positiveNetShares = 0;
  let positiveNetValueEok = 0;
  let negativeNetShares = 0;
  let positiveDays = 0;
  const recentRows = rows.slice(-40);
  let recentNetShares = 0;
  let recentNetValueEok = 0;

  for (const row of rows) {
    const shares = (row.foreignNetShares ?? 0) + (row.institutionNetShares ?? 0);
    const value = (row.foreignNetValueEok ?? 0) + (row.institutionNetValueEok ?? 0);
    netShares += shares;
    netValueEok += value;
    if (shares > 0 && value > 0) {
      positiveNetShares += shares;
      positiveNetValueEok += value;
      positiveDays += 1;
    } else if (shares < 0) {
      negativeNetShares += Math.abs(shares);
    }
  }

  for (const row of recentRows) {
    recentNetShares += (row.foreignNetShares ?? 0) + (row.institutionNetShares ?? 0);
    recentNetValueEok += (row.foreignNetValueEok ?? 0) + (row.institutionNetValueEok ?? 0);
  }

  const firstForeign = rows.find((row) => row.foreignHoldingRatePct != null)?.foreignHoldingRatePct ?? null;
  const latestForeign = rows.toReversed().find((row) => row.foreignHoldingRatePct != null)?.foreignHoldingRatePct ?? null;
  const estimatedCost = positiveNetShares > 0 ? positiveNetValueEok * 100_000_000 / positiveNetShares : null;
  const sellShareRatio = positiveNetShares > 0 ? negativeNetShares / positiveNetShares : null;
  const marketCapEok = quote.marketCapEok;
  const listedShares = quote.listedShares;

  return {
    days: rows.length,
    estimatedCost: estimatedCost == null ? null : round(estimatedCost, 0),
    positiveDays,
    netShares: Math.round(netShares),
    positiveNetShares: Math.round(positiveNetShares),
    negativeNetShares: Math.round(negativeNetShares),
    netValueEok: round(netValueEok, 2),
    positiveNetValueEok: round(positiveNetValueEok, 2),
    recentNetShares: Math.round(recentNetShares),
    recentNetValueEok: round(recentNetValueEok, 2),
    netValueToMarketCapPct: marketCapEok ? round(netValueEok / marketCapEok * 100, 3) : null,
    positiveValueToMarketCapPct: marketCapEok ? round(positiveNetValueEok / marketCapEok * 100, 3) : null,
    netSharesToListedPct: listedShares ? round(netShares / listedShares * 100, 3) : null,
    sellShareRatio: sellShareRatio == null ? null : round(sellShareRatio, 2),
    foreignHoldingChangePctp: firstForeign != null && latestForeign != null ? round(latestForeign - firstForeign, 3) : null,
    latestForeignHoldingRatePct: latestForeign,
    recentAccumulation: recentNetShares > 0 && recentNetValueEok > 0,
    disposalHistory: negativeNetShares > positiveNetShares * 0.45 || netShares < 0
  };
}

function applyHolderCostScores(rows) {
  for (const row of rows) {
    const holderCost = estimateHolderCost(row);
    row.holderCost = holderCost;
    row.holderCostScore = holderCost.score;
    row.holderSignal = holderCost.signal;
    row.v8cScore = round((row.totalScore ?? 0) + holderCost.score, 1);
    appendHolderCostMarker(row, holderCost);
  }
}

function estimateHolderCost(row) {
  const direct = HOLDER_COST_DIRECT_INPUTS.get(row.ticker);
  if (direct?.estimatedCost) {
    return finalizeHolderCost(row, {
      tier: "A",
      confidence: 1,
      estimatedCost: direct.estimatedCost,
      holder: direct.holder,
      sourceUrl: direct.sourceUrl,
      asOf: direct.asOf,
      basis: direct.memo ?? "DART direct transaction-price weighted average"
    });
  }

  const flow = row.holderFlow;
  const hasNamedPublicHolder = (row.megaManagers?.managerCount ?? 0) > 0 || Boolean(row.megaManagers?.largeHolderDisclosure);
  const hasForeignFivePctProxy = (flow?.latestForeignHoldingRatePct ?? 0) >= 5;
  const hasMaterialFlow = (flow?.netValueToMarketCapPct ?? 0) >= 0.5 || (flow?.netSharesToListedPct ?? 0) >= 0.1;
  if (flow?.estimatedCost && (hasNamedPublicHolder || hasForeignFivePctProxy || hasMaterialFlow)) {
    return finalizeHolderCost(row, {
      tier: "B",
      confidence: 0.7,
      estimatedCost: flow.estimatedCost,
      holder: hasNamedPublicHolder ? "공개 운용사/5% 보유 신호 + 외국인·기관 수급" : "외국인·기관 수급 프록시",
      sourceUrl: row.sourceUrl,
      asOf: row.latestDate,
      basis: `최근 ${flow.days}거래일 외국인+기관 순매수일 VWAP 추정`
    });
  }
  ...
  return {
    ...
    estimatedCost: null,
    gapPct: null,
    scoreBeforeConfidence: 0,
    score: 0,
    signal: "NO_DATA",
    holder: row.ownership?.majorHolderName ?? null,
    basis: "오너·창업자·프록시 보유는 평단가가 투자 판단에 직접 유효하지 않아 점수화하지 않음",
    components: { gap: 0, accumulation: 0, holderQuality: 0, overhangPenalty: 0 },
    memo: "대주주 평단 NO_DATA"
  };
}

function finalizeHolderCost(row, profile) {
  const flow = row.holderFlow ?? {};
  const gapPct = pctChange(profile.estimatedCost, row.close);
  const gapScore = holderCostGapScore(gapPct);
  const nearCost = gapPct != null && gapPct >= -15 && gapPct <= 30;
  let accumulation = 0;
  if (flow.recentAccumulation && nearCost) accumulation = 10;
  else if ((flow.netShares ?? 0) > 0 && gapPct != null && gapPct >= -20 && gapPct <= 35) accumulation = 6;
  else if ((flow.foreignHoldingChangePctp ?? 0) > 0.1) accumulation = 4;

  const holderQuality = (row.megaManagers?.managerCount ?? 0) > 0 || row.megaManagers?.largeHolderDisclosure ? 3 : 0;
  const overhangPenalty = gapPct >= 50 && flow.disposalHistory ? 10 : gapPct >= 35 && flow.disposalHistory ? 5 : 0;
  const raw = clamp(gapScore + accumulation + holderQuality - overhangPenalty, 0, 30);
  const score = round(raw * profile.confidence, 1);
  const signal = overhangPenalty >= 8
    ? "OVERHANG"
    : accumulation >= 6 && gapScore >= 8 ? "ACCUMULATION" : "NEUTRAL";

  return {
    ...profile,
    estimatedCost: round(profile.estimatedCost, 0),
    gapPct: round(gapPct, 2),
    scoreBeforeConfidence: raw,
    score,
    signal,
    components: { gap: gapScore, accumulation, holderQuality, overhangPenalty },
    flow: {
      days: flow.days,
      netValueToMarketCapPct: flow.netValueToMarketCapPct,
      positiveValueToMarketCapPct: flow.positiveValueToMarketCapPct,
      foreignHoldingChangePctp: flow.foreignHoldingChangePctp,
      sellShareRatio: flow.sellShareRatio,
      recentAccumulation: flow.recentAccumulation,
      disposalHistory: flow.disposalHistory
    },
    memo: `${profile.tier} ${formatPrice(profile.estimatedCost)} / 현재가 대비 ${formatPct(gapPct)} / 신뢰도 ${profile.confidence}`
  };
}

function holderCostGapScore(gapPct) {
  if (gapPct == null) return 0;
  if (gapPct >= -10 && gapPct <= 30) return 14;
  if (gapPct >= -20 && gapPct < -10) return 8;
  if (gapPct > 30 && gapPct <= 50) return 5;
  if (gapPct >= -40 && gapPct < -20) return 3;
  return 0;
}

function appendHolderCostMarker(row, holderCost) {
  if (!row.executionPlan || holderCost.estimatedCost == null) return;
  row.executionPlan.levels = [
    ...(row.executionPlan.levels ?? []),
    { label: "대주주 평단", kind: "holderCost", price: holderCost.estimatedCost }
  ].sort((a, b) => a.price - b.price);
  row.executionPlan.riskSwitches = [
    ...(row.executionPlan.riskSwitches ?? []),
    `대주주 평단 ${holderCost.signal}: ${holderCost.memo}`
  ];
}
...
<body>
  <div class="layout">
    <aside>
      <div class="brand"><h1>${escapeHtml(data.meta.title)}</h1><p>후보 선별, 진입 조건, 매도·축소 규칙</p></div>
      ...
      <nav class="nav-list">
        ...
        <a class="nav-link" href="#execution"><span>실행 전략</span><span class="tag">v7</span></a>
        <a class="nav-link" href="#holderCost"><span>대주주 평단</span><span class="tag">v8</span></a>
    ...
    <main>
      <section class="hero" id="overview"><div><p class="kicker">v8 Holder Cost Dashboard</p><h2>기존 진입 판단은 유지하고 대주주 평단 보조축을 추가합니다.</h2><p>${escapeHtml(data.meta.warning)}</p></div><div class="grid-2"><div class="card"><h4>KOSPI</h4><p><strong>${escapeHtml(data.market.kospi.now)}</strong></p><p>${escapeHtml(data.market.kospi.change)}</p></div><div class="card"><h4>KOSDAQ</h4><p><strong>${escapeHtml(data.market.kosdaq.now)}</strong></p><p>${escapeHtml(data.market.kosdaq.change)}</p></div></div></section>
      <section class="metrics" id="metrics"></section>
      <section class="band" id="execution"><div class="head"><div><h3>진입·매도 실행 전략</h3><p>진입 가능 후보는 매수 가격보다 축소·손절 조건을 먼저 고정해 봅니다.</p></div></div><div class="execution-grid" id="executionCards"></div></section>
      <section class="band" id="holderCost"><div class="head"><div><h3>대주주 평단 v8c 랭킹</h3><p>현재가는 추정 평단 대비 손익권을 보고, 점수는 신뢰도 계수를 곱해 보수적으로 반영합니다. v8cScore는 기존 totalScore에 holderCostScore를 더한 별도 랭킹입니다.</p></div></div><div class="table-wrap"><table><thead><tr><th>v8c 순위</th><th>회사</th><th>v8c</th><th>기존 판단</th><th>대주주 평단 축</th><th>추정 근거</th><th>수급/오버행</th></tr></thead><tbody id="holderRows"></tbody></table></div></section>
      ...
      <section class="band" id="entry"><div class="head"><div><h3>진입 가능 상세 표</h3><p>MA20 회복, RSI, 낙폭, 수급, 리스크 페널티를 통과한 후보입니다. 표에서는 대주주 평단·거래원·리스크까지 함께 확인합니다.</p></div></div><div class="table-wrap"><table><thead><tr><th>순위</th><th>회사</th><th>점수</th><th>v8c</th><th>가격/가치</th><th>대주주 평단</th><th>대주주</th><th>기술적 위치</th><th>수급</th><th>3대 운용사</th><th>거래원</th><th>진입 계획</th><th>매도 전략</th><th>리스크</th></tr></thead><tbody id="entryRows"></tbody></table></div></section>
      ...
      <footer>생성 스크립트: <code>node scripts/build-v8-holder-cost-dashboard.mjs</code>. 데이터: <code>data/v8-execution-dashboard-data.json</code>.</footer>
    </main>
  </div>
  <script>
    const DATA=${json}; let filter="all"; let search="";
    const megaStats=DATA.megaManagers?.summary??{};
    const holderStats=DATA.summary?.holderSignals??{};
    const metrics=[["분석 유니버스",DATA.meta.validCount+"개","기존 v4 순위 배제"],["실행 전략",DATA.entryList.length+"개","진입·매도 동시 표시"],["대주주 평단",(holderStats.accumulation??0)+"개","ACCUMULATION"],["트리거 대기",DATA.summary.waitTrigger+"개","회복 확인 필요"],["운용사 신호",(megaStats.withAny??0)+"개","BlackRock·Vanguard·SSGA 매칭"]];
    ...
    document.querySelector("#holderRows").innerHTML=(DATA.holderRankList||[]).map((r,i)=>\`<tr><td class="num">\${i+1}</td><td><span class="company">\${escapeHtml(r.company)}</span><span class="note">\${r.ticker} · \${escapeHtml(r.sector)}</span></td><td><span class="badge holder">\${num(r.v8cScore)}</span><span class="note">기존 \${num(r.totalScore)} + 평단 \${num(r.holderCostScore)}</span></td><td><span class="badge \${decisionClass(r.decision)}">\${decisionText(r.decision)}</span></td><td>\${holderCell(r)}</td><td>\${escapeHtml(r.holderCost?.basis??"")}<span class="note">Tier \${escapeHtml(r.holderCost?.tier??"-")} · 신뢰도 \${num(r.holderCost?.confidence)}</span></td><td>\${holderFlowCell(r)}</td></tr>\`).join("");
    ...
    document.querySelector("#entryRows").innerHTML=DATA.entryList.map((r,i)=>\`<tr><td class="num">\${i+1}</td><td><span class="company">\${escapeHtml(r.company)}</span><span class="note">\${r.ticker} · \${escapeHtml(r.sector)}</span></td><td><span class="badge entry">\${r.totalScore}</span></td><td><span class="badge holder">\${num(r.v8cScore)}</span><span class="note">+\${num(r.holderCostScore)}</span></td><td>\${price(r.close)}<span class="note">시총 \${eok(r.marketCapEok)} · PER \${r.per??"-"} · PBR \${r.pbr??"-"}</span></td><td>\${holderCell(r)}</td><td>\${ownershipCell(r)}</td><td>\${escapeHtml(r.technical.memo)}<span class="note">20일 \${pct(r.returns.d20)}, 60일 \${pct(r.returns.d60)}</span></td><td>\${escapeHtml(r.flowScore.memo)}</td><td>\${managerCell(r)}</td><td>\${escapeHtml(broker(r))}</td><td>\${escapeHtml(r.entryPlan.trigger)}<span class="note">\${escapeHtml(r.entryPlan.invalidation)}</span></td><td>\${sellSummary(r)}</td><td>\${escapeHtml((r.risk.notes||[]).join(" · ")||"특이 리스크 없음")}</td></tr>\`).join("");
    ...
    function renderAll(){const needle=search.trim().toLowerCase();const rows=DATA.allRows.filter(r=>(filter==="all"||r.decision===filter)&&(!needle||[r.company,r.ticker,r.sector,r.rationale].join(" ").toLowerCase().includes(needle))).slice(0,90);document.querySelector("#allRows").innerHTML=rows.map((r,i)=>\`<tr><td class="num">\${i+1}</td><td><span class="company">\${escapeHtml(r.company)}</span><span class="note">\${r.ticker} · \${escapeHtml(r.sector)}</span></td><td><span class="badge \${decisionClass(r.decision)}">\${decisionText(r.decision)}</span></td><td><strong>\${r.totalScore}</strong></td><td><strong>\${num(r.v8cScore)}</strong><span class="note">평단 +\${num(r.holderCostScore)}</span></td><td>\${r.policy.score}<span class="note">\${escapeHtml(r.policy.memo)}</span></td><td>\${r.value.score}<span class="note">\${escapeHtml(r.value.memo)}</span></td><td>\${holderCell(r)}</td><td>\${r.technical.score}<span class="note">\${escapeHtml(r.technical.memo)}</span></td><td>\${r.flowScore.score}<span class="note">\${escapeHtml(r.flowScore.memo)}</span></td><td>\${managerCell(r)}</td><td>\${escapeHtml(r.entryPlan.action)}<span class="note">\${escapeHtml((r.risk.notes||[]).join(" · "))}</span></td></tr>\`).join("");}
    ...
    function strategyCard(row,index){const p=row.executionPlan??{};return \`<article class="strategy-card"><div class="strategy-head"><div><h4>\${index+1}. \${escapeHtml(row.company)}</h4><span class="note">\${row.ticker} · \${escapeHtml(row.sector)} · 현재 \${price(row.close)}</span></div><div class="strategy-meta"><span class="pill good">\${escapeHtml(p.stance??row.entryPlan.action)}</span><span class="pill \${p.isOverheated?"warn":"good"}">\${p.isOverheated?"과열 축소":"분할 기준"}</span><span class="pill \${p.isWeakFlow?"bad":"good"}">\${p.isWeakFlow?"수급 약함":"수급 양호"}</span><span class="pill holder">\${holderSignalText(row.holderSignal)} \${num(row.holderCostScore)}점</span></div></div>\${priceRail(p)}<div class="plan-columns"><div><h5>진입</h5>\${stepList(p.buySteps,"buy")}</div><div><h5>매도·축소</h5>\${stepList(p.sellSteps,"sell")}</div></div><div class="session-grid">\${(p.sessionRules||[]).map(x=>\`<div class="session-item"><strong>\${escapeHtml(x.window)}</strong><span>\${escapeHtml(x.rule)}</span></div>\`).join("")}</div><div class="risk-strip">\${(p.riskSwitches||[]).map(x=>\`<span>\${escapeHtml(x)}</span>\`).join("")}</div></article>\`}
    ...
    function holderCell(row){const h=row.holderCost;if(!h||h.signal==="NO_DATA")return \`<span class="badge nodata">NO_DATA</span><span class="note">\${escapeHtml(h?.basis??"공개 평단 근거 없음")}</span>\`;return \`<span class="badge \${holderSignalClass(h.signal)}">\${holderSignalText(h.signal)}</span><span class="note"><strong>\${price(h.estimatedCost)}</strong> · 현재가 대비 \${pct(h.gapPct)}</span><span class="note">Tier \${escapeHtml(h.tier)} · \${num(h.score)}점 · \${escapeHtml(h.holder??"")}</span>\`}
    function holderFlowCell(row){const f=row.holderCost?.flow??row.holderFlow??{};return \`<span class="note">순매수/시총 \${pct(f.netValueToMarketCapPct)} · 매집/시총 \${pct(f.positiveValueToMarketCapPct)}</span><span class="note">외국인 지분 변화 \${pct(f.foreignHoldingChangePctp)}p · 매도비율 \${num(f.sellShareRatio)}</span>\`}
    function holderSignalClass(v){return v==="OVERHANG"?"overhang":v==="NO_DATA"?"nodata":"holder"}
    function holderSignalText(v){return v==="ACCUMULATION"?"ACCUMULATION":v==="OVERHANG"?"OVERHANG":v==="NO_DATA"?"NO_DATA":"NEUTRAL"}
