...
const dataPath = path.join(root, "data", "v7-execution-dashboard-data.json");
const outputPath = path.join(root, "docs", "v7.html");
...
async function main() {
  ...
  const output = {
    meta: {
      title: "국민성장펀드 v7 진입·매도 실행 대시보드",
      runDate: RUN_DATE,
      universeCount: universe.length,
      validCount: valid.length,
      methodology: "기존 v4 우선순위를 배제하고 정책자금 잔여 버킷, 유동성·시총 안정성, 급락 후 기술적 안정화, 외국인·기관 수급, 거래원 보조 신호를 종합 평가했다. v7은 진입 가능 후보에 대해 장초반 급등, 지수 약세, 고점 반락 상황에서의 매도·축소 규칙을 함께 표시한다.",
      warning: "투자 권유가 아니라 후보 탐색과 실행 조건 점검표입니다. ENTRY_OK도 시장 약세와 장중 윗꼬리 발생 시 신규 진입보다 부분 회수·관망이 우선입니다."
    },
...
<body>
  <div class="layout">
    ...
    <main>
      ...
      <section class="band" id="entry"><div class="head"><div><h3>진입 가능 상세 표</h3><p>MA20 회복, RSI, 낙폭, 수급, 리스크 페널티를 통과한 후보입니다. 표에서는 대주주·거래원·리스크까지 함께 확인합니다.</p></div></div><div class="table-wrap"><table><thead><tr><th>순위</th><th>회사</th><th>점수</th><th>가격/가치</th><th>대주주</th><th>기술적 위치</th><th>수급</th><th>3대 운용사</th><th>거래원</th><th>진입 계획</th><th>매도 전략</th><th>리스크</th></tr></thead><tbody id="entryRows"></tbody></table></div></section>
      ...
      <footer>생성 스크립트: <code>node scripts/build-v7-execution-dashboard.mjs</code>. 데이터: <code>data/v7-execution-dashboard-data.json</code>.</footer>
    </main>
  </div>
  <script>
    const DATA=${json}; let filter="all"; let search="";
    const megaStats=DATA.megaManagers?.summary??{};
    const metrics=[["분석 유니버스",DATA.meta.validCount+"개","기존 v4 순위 배제"],["실행 전략",DATA.entryList.length+"개","진입·매도 동시 표시"],["트리거 대기",DATA.summary.waitTrigger+"개","회복 확인 필요"],["관망/제외",DATA.summary.avoidNow+"개","추세·리스크 미통과"],["운용사 신호",(megaStats.withAny??0)+"개","BlackRock·Vanguard·SSGA 매칭"]];
